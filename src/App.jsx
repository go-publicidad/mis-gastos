...
export default function App() {
  ...
  const loadData = async (isManual = false) => {
    ...
    try {
      ...
      if (cfg) {
        // SOLUCIÓN BUG CLAUDE: Buscamos la llave simple, ignorando prefijos antiguos si existen
  ...
  const eliminar = async (g) => {
    ...
    if (g.tipo === "aporte") {
        ...
        if (metaEncontrada) {
            ...
            // ACTUALIZACIÓN SEGURA: onConflict por user_id y key
  ...
  const guardarCatForm = async () => {
    ...
    setSaving(true);
    await supabase.from("config").upsert([{ user_id: usuario.id, key, value: JSON.stringify(nuevoArray) }], { onConflict: "user_id, key" });
  ...
  return (
    <div style={s.app}>
      ...
      {metaSeleccionada && (() => {
        ...
        const ahorroDiarioVal = diasRestantes > 0 ? (faltan / diasRestantes) : 0; const bgIconColor = PASTEL_COLORS[Math.abs(metaSeleccionada.nombre.length) % PASTEL_COLORS.length];
        ...
        return (
          <div className="hide-scroll" style={{ position: "fixed", inset: 0, background: c.bg, zIndex: 10000, display: "flex", flexDirection: "column", animation: isClosing === 'detalleMeta' ? "slideOutToLeft 0.28s cubic-bezier(0.25, 0.8, 0.25, 1) forwards" : "slideInFromLeft 0.3s cubic-bezier(0.25, 0.8, 0.25, 1) forwards" }}>
            ...
            <div style={{ padding: "32px 20px 20px", flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
              ...
              <button onClick={() => { setIsEditingMetaObj(true); setMetaForm(metaSeleccionada); setShowCrearMeta(true); setMetaSeleccionada(null); }} style={{ width: "100%", padding: 16, background: "#059669", color: "#FFF", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 1205, 150, 105, 0.2)" }}>Editar meta</button>
            </div>
          </div>
        );
      })()}
